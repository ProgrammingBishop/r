...
# Install Packages
# ==================================================
packages <- c("DescTools", "tree", "glmnet", "randomForest", "xgboost")   
...
# Fit New Model
model.cv <- cv.glmnet( as.matrix( dummy.train.x[, 3:ncol(dummy.train.x) ] ), 
                       as.matrix( sig.train.y ), 
                       alpha = 1, lambda = lambda.values.l )
                          
best.lambda <- model.cv$lambda.min
predictions <- round( predict( model.cv, s = best.lambda, newx = dummy.test.x[, 3:ncol(dummy.test.x) ] ), 2 )

# Get MSPE
sqrt( mean( ( log(predictions) - log(sig.test.y) ) ^ 2 ) )
...
# Random Forest
rf.data    <- cbind(sig.train.y, sig.train.x[, -1])
rf.model   <- randomForest( sig.train.y ~ ., data = rf.data, ntree = 500 )
rf.predict <- predict( rf.model, sig.test.x[, -1] )

sqrt( mean( ( log(rf.predict) - log(sig.test.y) ) ^ 2 ) )
...
xg.predict <- predict( xg.train.1, xg.test )
